          ...
          {revenueByDay.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={revenueByDay}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#14181f99' }} />
                <YAxis tick={{ fontSize: 11, fill: '#14181f99' }} />
                <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #e4e1d9', fontSize: 12 }} />
                <Line type="monotone" dataKey="total" stroke="#0f6e4f" strokeWidth={2} dot={{ fill: '#0f6e4f', r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </motion.div>
          ...
          {ordersByStatus.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={ordersByStatus}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e4e1d9" />
                <XAxis dataKey="status" tick={{ fontSize: 11, fill: '#14181f99' }} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: '#14181f99' }} />
                <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #e4e1d9', fontSize: 12 }} />
                <Bar dataKey="count" fill="#0f6e4f" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </motion.div>
          ...
          {topProducts.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={topProducts} layout="vertical" margin={{ left: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e4e1d9" />
                <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: '#14181f99' }} />
                <YAxis type="category" dataKey="name" width={120} tick={{ fontSize: 11, fill: '#14181f99' }} />
                <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #e4e1d9', fontSize: 12 }} />
                <Bar dataKey="qty" fill="#c97a2b" radius={[0, 6, 6, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </motion.div>
