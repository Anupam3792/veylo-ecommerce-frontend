...
const HERO_SLIDES = [
  {
    ...
    subtitle: "Veylo tracks live inventory on every product, so what you see is what's actually on the shelf.",
  ...
  },
  {
    ...
    cta: 'Start shopping',
    to: '/catalog',
    theme: 'from-ink via-[#1a3a2e] to-verdant',
    icon: Gift,
  },
  {
    id: 'delivery',
...
function BrandMarquee({ brands }) {
  ...
  return (
    <div className="overflow-hidden">
      <style>{`
        @keyframes veylo-brand-marquee {
          from { transform: translateX(0); }
          to { transform: translateX(-50%); }
        }
        .veylo-brand-track {
          animation: veylo-brand-marquee 30s linear infinite;
        }
        .veylo-brand-track:hover {
          animation-play-state: paused;
        }
      `}</style>
      <div className="flex gap-4 w-max veylo-brand-track">
        {loop.map((brand, i) => (
          <Link
            key={`${brand}-${i}`}
