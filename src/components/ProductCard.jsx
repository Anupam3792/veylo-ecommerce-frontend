...
function stockStatus(stock) {
  ...
  if (stock <= 10) return { label: `Only ${stock} left`, dot: 'bg-ember', text: 'text-ember', pulse: true };
...
export default function ProductCard({ product }) {
  ...
  // Second gallery image (if any) used for the hover image-swap effect.
      ...
      >
        <div className="aspect-square rounded-2xl bg-stone overflow-hidden relative shadow-sm group-hover:shadow-xl transition-shadow duration-300">
          <div className="absolute top-3 left-3 z-10 flex flex-col items-start gap-1.5">
            {product.isNew && (
              <span className="bg-ink text-paper text-[11px] font-mono px-2 py-1 rounded-full">NEW</span>
            )}
            {product.bestSeller && (
              <span className="bg-verdant text-paper text-[11px] font-mono px-2 py-1 rounded-full">BESTSELLER</span>
            )}
            {hasDiscount && (
          ...
          >
            <Heart size={15} strokeWidth={1.75} className={wishlisted ? 'text-ember fill-ember' : 'text-ink/50'} />
          </motion.button>
          ...
          <div className="absolute bottom-3 right-3 z-10 flex items-center gap-2">
            ...
            <motion.button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 disabled:opacity-40 ${
                added ? 'bg-verdant opacity-100' : 'bg-ink text-paper opacity-0 group-hover:opacity-100'
              }`}
              aria-label={`Add ${product.name} to cart`}
            >
              {added ? <Check size={16} strokeWidth={2} className="text-paper" /> : <ShoppingBag size={16} strokeWidth={1.75} />}
            </motion.button>
          </div>
        </div>
        ...
        <div className="mt-4">
          {product.brand && (
            <p className="text-[11px] uppercase tracking-widest text-ink/40">{product.brand}</p>
          )}
          <h3 className="font-display text-lg leading-snug mt-0.5">{product.name}</h3>

          <div className="flex items-center gap-2 mt-1">
            <span className="font-mono text-sm">₹{product.price.toFixed(2)}</span>
            {hasDiscount && (
              <span className="font-mono text-xs text-ink/40 line-through">₹{product.originalPrice.toFixed(2)}</span>
            )}
          </div>
          ...
          {product.rating > 0 && (
            <div className="flex items-center gap-1.5 mt-1">
              <span className="flex items-center gap-0.5 bg-verdant text-paper text-[11px] font-mono px-1.5 py-0.5 rounded">
                {product.rating.toFixed(1)} <Star size={10} fill="currentColor" strokeWidth={0} />
              </span>
              <span className="text-[11px] text-ink/40 font-mono">({product.reviewCount})</span>
            </div>
          )}

          <p className="text-xs uppercase tracking-widest text-ink/40 mt-1.5">{product.category}</p>
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <h2 className="font-display text-2xl leading-snug mt-0.5 pr-8">{product.name}</h2>
                    ...
                    {product.rating > 0 && (
                      <div className="flex items-center gap-1.5 mt-2">
                        ...
                        <span className="text-[11px] text-ink/40 font-mono">({product.reviewCount} ratings)</span>
                      </div>
                    )}
                    ...
                    <div className="flex items-baseline gap-2 mt-3">
                      ...
                      {hasDiscount && (
                        <>
                          ...
                          <span className="font-mono text-xs text-ember">({discountPercent}% OFF)</span>
                        </>
                      )}
                    </div>
                    ...
                    {product.description && (
                      <p className="text-sm text-ink/60 leading-relaxed mt-4 line-clamp-4">{product.description}</p>
                    )}
                    ...
                    <div className="mt-auto pt-6 flex flex-col gap-2.5">
                      <motion.button
                        ...
                        className={`flex items-center justify-center gap-2 py-3 rounded-full text-sm font-sans uppercase tracking-widest transition-colors disabled:opacity-40 ${
                          quickViewAdded ? 'bg-verdant text-paper' : 'bg-ink text-paper hover:bg-ink-light'
                        }`}
                      >
                        {quickViewAdded ? (
                          <>
