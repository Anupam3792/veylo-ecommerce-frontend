          ...
          >
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (
                <p className="text-center text-ink/40 font-mono text-sm py-10">No notifications yet.</p>
              ) : (
                notifications.map((n) => (
                  <div
                    key={n.id}
                    className={`flex items-start gap-3 px-4 py-3 border-b border-stone/60 last:border-0 ${!n.read ? 'bg-verdant/5' : ''}`}
                  >
                    <div className="w-9 h-9 rounded-full bg-verdant/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <ShoppingBag size={15} strokeWidth={1.75} className="text-verdant" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm">
                        New order <span className="font-mono">#{n.orderId}</span>
                      </p>
                      <p className="text-xs text-ink/50 mt-0.5">
                        {n.customerName} · ₹{n.totalAmount.toFixed(2)}
                      </p>
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-[11px] text-ink/30 font-mono">{timeAgo(n.createdAt)}</span>
                        <button
                    ...
                    {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-ember mt-2 flex-shrink-0" />}
                  </div>
                ))
              )}
            </div>
